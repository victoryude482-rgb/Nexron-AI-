...
export function memoryStoreFromEnv(env:Record<string,string|undefined>=process.env):MemoryStore{const url=env.SUPABASE_URL?.trim();const key=env.SUPABASE_SERVICE_ROLE_KEY?.trim();if(url&&key)return new SupabaseMemoryStore(createClient(url,key));return new InMemoryMemoryStore();}
