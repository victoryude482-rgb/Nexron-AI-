...
export async function POST(request:Request){
 ...
 const stream=new ReadableStream({async start(out){try{out.enqueue(encoder.encode("event: status\ndata: {"status":"running"}\n\n"));for await(const delta of getRuntime().stream(body.message.trim(),{signal:controller.signal,userId:auth.userId})){out.enqueue(encoder.encode("event: delta\ndata: "+JSON.stringify({delta})+"\n\n"));}out.enqueue(encoder.encode("event: done\ndata: {}\n\n"));out.close();}catch(error){out.enqueue(encoder.encode("event: error\ndata: "+JSON.stringify({error:error instanceof Error?error.message:String(error)})+"\n\n"));out.close();}}});
