...
function leadService(){const endpoint=process.env.LEADPILOT_URL?.trim();const providers=endpoint?[new LeadHttpProvider({name:"leadpilot",endpoint,apiKey:process.env.LEADPILOT_API_KEY,searchPath:process.env.LEADPILOT_SEARCH_PATH})]:[];return new ProviderBackedLeadIntelligence(providers);}
